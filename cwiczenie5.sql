...
CREATE TABLE bujak.intersects AS
...
alter table bujak.intersects
add column rid SERIAL PRIMARY KEY;
-- 2. utworzenie indeksu przestrzennego:

CREATE INDEX idx_intersects_rast_gist ON bujak.intersects
USING gist (ST_ConvexHull(rast));
-- 3. dodanie raster constraints:

SELECT AddRasterConstraints('bujak'::name, 'intersects'::name,'rast'::name);

-- Przykład 2 - ST_Clip
-- Obcinanie rastra na podstawie wektora.


CREATE TABLE bujak.clip AS
...
DROP TABLE bujak.porto_parishes;
CREATE TABLE bujak.porto_parishes AS
WITH r AS (
...
DROP TABLE bujak.porto_parishes; --> drop table porto_parishes first
CREATE TABLE bujak.porto_parishes AS
WITH r AS (
SELECT rast FROM rasters.dem
LIMIT 1
...
create table bujak.intersection as
...
CREATE TABLE bujak.dumppolygons AS
...
CREATE TABLE bujak.landsat_nir AS
SELECT rid, ST_Band(rast,4) AS rast
FROM rasters.landsat8;

-- Przykład 2 - ST_Clip

CREATE TABLE bujak.paranhos_dem AS
...
CREATE TABLE bujak.paranhos_slope AS
SELECT a.rid,ST_Slope(a.rast,1,'32BF','PERCENTAGE') as rast
FROM bujak.paranhos_dem AS a;


-- Przykład 4 - ST_Reclass

CREATE TABLE bujak.paranhos_slope_reclass AS
SELECT a.rid,ST_Reclass(a.rast,1,']0-15]:1, (15-30]:2, (30-9999:3', '32BF',0)
FROM bujak.paranhos_slope AS a;
...
create table bujak.tpi30 as
select ST_TPI(a.rast,1) as rast
from rasters.dem a;

CREATE INDEX idx_tpi30_rast_gist ON bujak.tpi30
USING gist (ST_ConvexHull(rast));

SELECT AddRasterConstraints('bujak'::name, 'tpi30'::name,'rast'::name);

-- Problem do samodzielnego rozwiązania

create table bujak.tpi30_porto as
SELECT ST_TPI(a.rast,1) as rast
FROM rasters.dem AS a, vectors.porto_parishes AS b
WHERE ST_Intersects(a.rast, b.geom) AND b.municipality ilike 'porto';

CREATE INDEX idx_tpi30_porto_rast_gist ON bujak.tpi30_porto
USING gist (ST_ConvexHull(rast));

SELECT AddRasterConstraints('bujak'::name, 'tpi30_porto'::name,'rast'::name);


-- Algebra map
-- Przykład 1 - Wyrażenie Algebry Map

CREATE TABLE bujak.porto_ndvi AS
WITH r AS (
SELECT a.rid,ST_Clip(a.rast, b.geom,true) AS rast
FROM rasters.landsat8 AS a, vectors.porto_parishes AS b
WHERE b.municipality ilike 'porto' and ST_Intersects(b.geom,a.rast)
...
CREATE INDEX idx_porto_ndvi_rast_gist ON bujak.porto_ndvi
USING gist (ST_ConvexHull(rast));

SELECT AddRasterConstraints('bujak'::name, 'porto_ndvi'::name,'rast'::name);

-- Przykład 2 – Funkcja zwrotna

create or replace function bujak.ndvi(
...
CREATE TABLE bujak.porto_ndvi2 AS
WITH r AS (
SELECT a.rid,ST_Clip(a.rast, b.geom,true) AS rast
FROM rasters.landsat8 AS a, vectors.porto_parishes AS b
WHERE b.municipality ilike 'porto' and ST_Intersects(b.geom,a.rast)
...
r.rid,ST_MapAlgebra(
r.rast, ARRAY[1,4],
'bujak.ndvi(double precision[], integer[],text[])'::regprocedure, --> This is the function!
'32BF'::text
) AS rast
FROM r;

CREATE INDEX idx_porto_ndvi2_rast_gist ON bujak.porto_ndvi2
USING gist (ST_ConvexHull(rast));
